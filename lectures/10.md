...
3. Выполняем шаг алгоритма:
    ...
    $$
4. Вычисляем новое направление метода так, чтобы $d_{k+1}$ было $A$-ортогонально всем предыдущим направлениям метода. Позднее, мы покажем, что в случае метода сопряженных градиентов (CG), для этого достаточно хранить в памяти только последнее направление. В общем случае (CD) это не так. 
