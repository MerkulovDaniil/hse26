---
title: "Субградиент. Субдифференциал. Субградиентный спуск. Теоремы сходимости в негладком случае."
...
[![](../files/l1_regularization.jpeg)](https://fmin.xyz/assets/Notebooks/Regularization_horizontal.mp4)
...
![Конусы норм для разных $p$: нормы негладкие](../files/norm_cones.pdf){width=90%}

## Пример Вульфа

![Пример Вульфа](../files/wolfe_3d.pdf){width=90%}
...
::: {.column width="60%"}
![Линейная аппроксимация по Тейлору является глобальной нижней оценкой выпуклой функции](../files/subgrad.pdf)
:::
...
![Субдифференциал — множество всех возможных субградиентов](../files/Subdifferential.pdf)
...
![Субдифференциал функции $\vert x \vert$](../files/subgradmod.pdf){width=85%}
...
![](../files/subgrad_step_sizes.pdf){fig-align="center" width=88%}

## Различные стратегии выбора шага

![](../files/subgrad_fraction.pdf){fig-align="center" width=88%}
...
![](../files/non_smooth_convex_strongly_convex_ru.pdf){fig-align="center"}
...
![Гладкий выпуклый случай. Сублинейная сходимость, нет сходимости по аргументу](../files/lasso_m_1000_n_100_mu_0_lambda_0.pdf)
...
![Негладкий выпуклый случай. Малое значение $\lambda$ делает задачу негладкой. Нет сходимости при постоянном шаге](../files/lasso_m_1000_n_100_mu_0_lambda_0.1.pdf)
...
![Негладкий выпуклый случай. Большее значение $\lambda$ проявляет немонотонность $f(x_k)$. Меньший постоянный шаг приводит к более низкому стационарному уровню](../files/lasso_m_1000_n_100_mu_0_lambda_1.pdf)
...
![Негладкий выпуклый случай. Убывающий шаг обеспечивает сходимость $f^{\text{best}}_k$](../files/lasso_m_100_n_100_mu_0_lambda_1_k.pdf)
...
![Негладкий выпуклый случай. Шаг $\frac{\alpha_0}{\sqrt{k}}$ обеспечивает сходимость $f^{\text{best}}_k$](../files/lasso_m_100_n_100_mu_0_lambda_1_sqrtk.pdf)
...
![Негладкий выпуклый случай. Шаг $\frac{\alpha_0}{\sqrt{k}}$ обеспечивает сходимость $f^{\text{best}}_k$](../files/lasso_m_100_n_100_mu_0_lambda_1_sqrtk_long.pdf)
...
![Негладкий сильно выпуклый случай. Шаг $\frac{\alpha_0}{k}$ обеспечивает сходимость $f^{\text{best}}_k$](../files/lasso_m_100_n_100_mu_1_lambda_1_1_k.pdf)
...
![Негладкий сильно выпуклый случай. Шаг $\frac{\alpha_0}{\sqrt{k}}$ работает хуже](../files/lasso_m_100_n_100_mu_1_lambda_1_1_sqrtk.pdf)
...
![Логистическая регрессия с $\ell_1$-регуляризацией](../files/logreg_lasso_small_short.pdf)
...
![Логистическая регрессия с $\ell_1$-регуляризацией](../files/logreg_lasso_small_long.pdf)
...
![Логистическая регрессия с $\ell_1$-регуляризацией](../files/logreg_lasso_medium_short.pdf)
...
![Логистическая регрессия с $\ell_1$-регуляризацией](../files/logreg_lasso_medium_long.pdf)
...
![Логистическая регрессия с $\ell_1$-регуляризацией](../files/logreg_lasso_high_short.pdf)
...
![Логистическая регрессия с $\ell_1$-регуляризацией](../files/logreg_lasso_high_long.pdf)
...
| выпуклая (негладкая) | гладкая (невыпуклая) | гладкая & выпуклая | гладкая & сильно выпуклая (или PL) |
|:-----:|:-----:|:----:|:---------:|
...
* Заметим, что $0 \in \partial f(y)$:
 $$
    \begin{aligned}
    \partial f(y) &= \alpha y + \beta \,\text{conv}\left\{e_i \mid i: y[i] =  \max_{1 \leq j \leq k} y[j] \right\} \\
    &= \alpha y + \beta \,\text{conv}\left\{e_1, \dots, e_k \right\} \\
    0 &\in \partial f(y).
    \end{aligned}
    $$
...
### Выпуклый случай
$$
\alpha = \frac{G}{R}\frac{1}{1 + \sqrt{k}} \quad \beta = \frac{G\sqrt{k}}{1 + \sqrt{k}}
$$
...
$$
Заметим, в частности, что $\|y\|_2^2 = \frac{\beta^2}{\alpha^2 k} = \frac{G^2}{4\alpha^2 k} = \frac{R^2}{k} \leq R^2$ при этих параметрах
...
![Иллюстрация](../files/SD_gap.pdf){width=60%}
...
![Субградиентный метод для SVM. Сравнение различных стратегий выбора шага](../files/svm_hinge_subgrad.pdf)

## Метод опорных векторов. Немонотонность

![Немонотонность $f(\theta_k)$ при постоянном шаге. Убывающий шаг уменьшает осцилляции. Сходимость по аргументу зависит от стратегии](../files/svm_nonmonotone.pdf)
...
![Фактическая сходимость $f_k^{\text{best}} - f^*$ и теоретическая верхняя оценка для выпуклого и сильно выпуклого случаев. Наклоны совпадают: $\mathcal{O}(1/\sqrt{k})$ и $\mathcal{O}(1/k)$](../files/convergence_rate_theory_vs_practice.pdf)
...
![Шаг Поляка существенно превосходит стандартные стратегии и обеспечивает сходимость по аргументу. Однако требует знания $f^*$](../files/subgrad_polyak_step.pdf)
...
![С ростом размерности $n$ субградиентный метод сходится медленнее. На лог-лог графике все кривые параллельны прямой $1/\sqrt{k}$](../files/subgrad_dimension_effect.pdf)
