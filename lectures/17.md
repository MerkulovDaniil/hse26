...
Заменим скаляры на матрицы: $x_k \to X_k$, $a \to X_k^\top X_k$:
$$
\boxed{\;X_{k+1} = \tfrac{1}{2}X_k(3I - X_k^\top X_k)\;}
$$
...
Неподвижная точка: $\sigma = 1$ $\Rightarrow$ $\sigma_i(X_k) \to 1$ $\Rightarrow$ $X_k \to UV^\top$. Множитель $G$ «вшит» в $X_0$; итерация лишь выравнивает спектр, не трогая направления.
...
* Загоняют $\sigma_i$ в окрестность 1 за 5 шагов; к $UV^\top$ точно не сходятся (см. далее)
:::
::::
...
Magma превосходит Adam, Muon, Adafactor, GaLore на всех масштабах. На 1B: perplexity **13.8** vs 16.3 (Adam).
