...
Ни одного обращения матрицы — только умножения. Именно так первые ЭВМ реализовывали деление на аппаратном уровне.

. . .

Для **ортогонализации** (полярный фактор $UV^\top$) аналогичная идея:
$$
X_{k+1} = \tfrac{1}{2}X_k(3I - X_k^\top X_k), \qquad \sigma_i(X_k) \to 1.
$$
