...
**Данные**: 400 точек, две переплетённые спирали — классическая нелинейно-разделимая задача.

**Модель**: $x \in \mathbb{R}^3$ (2D + bias) $\to$ hidden (64, tanh) $\to$ output (1, sigmoid). BCE loss, mini-batch = 40, 3000 шагов.

**Ключевой дизайн**: $W_1 \in \mathbb{R}^{64 \times 3}$ — матричный параметр. Muon ортогонализует именно его. $W_2$ оптимизируется AdamW у всех методов (изолируем эффект на матричном слое).

## Классификация спиралей: гиперпараметры по умолчанию
...
Гиперпараметры по умолчанию (lr, momentum и т.д.) — **без тюнинга**. SGD и Nesterov отстают, AdamW и Muon сопоставимы.

## Классификация спиралей: после Optuna

500 trials × 1000 steps на каждый оптимизатор. Результаты после тюнинга:
...
* У SGD/Nesterov **learning rate доминирует** (>70% важности по Optuna).
* У Muon **weight decay и lr равнозначны** (~50%/40%) — ортогонализация снижает чувствительность к шагу.
