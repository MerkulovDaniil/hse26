---
title: "Нижние оценки и ускоренные градиентные методы. Метод тяжёлого шарика. Ускоренный градиентный метод Нестерова"
...
5. Наконец, мы получаем:
    ...
    &\leq 2 \exp\left( - \frac{k}{2}\sqrt{\frac{2}{\varkappa-1}} \right) \|e_0\|
    \end{aligned}
    $$
    (в последнем шаге используем $1+x \geq e^{x/2}$ при $0 \leq x \leq 2{,}5$)
