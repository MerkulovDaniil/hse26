...
$$\alpha_k = \alpha_0 \cdot \gamma^{[k/T]}, \quad \gamma = 0.1$$
...
- **Дисперсия mini-batch**: $\mathbb{E}\lVert g_\mathcal{B} - \nabla f\rVert^2 = \sigma^2/b$
- **Базовое неравенство**: $\mathbb{E}\lVert w_{k+1}-w^*\rVert^2 \leq \lVert w_k-w^*\rVert^2 - 2\alpha_k(f(w_k)-f^*) + \alpha_k^2 G^2$
...
$$\mathbb{E}\lVert w_k - w^*\rVert^2 \leq (1-\mu\alpha)^k \lVert w_0 - w^*\rVert^2 + \frac{\alpha G^2}{\mu}$$
