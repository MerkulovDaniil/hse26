...
![[Источник](https://twitter.com/gabrielpeyre/status/1273842947704999936)](../files/prox_vis.jpeg){width=63%}
...
::: {.column width="50%"}
![Оператор мягкого порога: «мёртвая зона» $[-\lambda, \lambda]$](../files/prox_l1.pdf)
:::
::::
...
::: {.column width="50%"}
![Линейное сжатие для разных $\lambda$](../files/prox_l2.pdf)
:::
::::
...
::: {.column width="50%"}
![Проекция на отрезок $[-a, a]$ — скалярный пример](../files/prox_proj.pdf)
:::
::::
...
![Субградиентный метод сходится как $\mathcal{O}(1/\sqrt{k})$, проксимальный — как $\mathcal{O}(1/k)$, FISTA — как $\mathcal{O}(1/k^2)$. Пунктир — теоретические оценки из доказанных теорем. Проксимальный метод в $\sqrt{k}$ раз быстрее субградиентного при одинаковой стоимости итерации.](../files/prox_convex_theory.pdf)

## LASSO: сильно выпуклый случай — теоретические гарантии

![Проксимальный метод достигает линейной сходимости $(1-\mu/L)^k$ даже для негладкой задачи. Субградиентный метод сходится лишь сублинейно $\mathcal{O}(1/k)$. Разница порядков в логарифмической шкале колоссальна.](../files/prox_sc_theory.pdf)

## LASSO: разреженность решения

![Проксимальный метод порождает точные нули (21/30 компонент), в точности воспроизводя структуру оптимального решения $x^*$. Субградиентный метод даёт малые, но ненулевые значения (0 точных нулей) — soft-thresholding принципиально отличается от вычитания субградиента.](../files/prox_sparsity.pdf)

## Логистическая регрессия с $\ell_1$-регуляризацией

![Проксимальный метод и FISTA сходятся за десятки итераций, субградиентный — за тысячи. Проксимальный подход работает не только для квадратичных задач: достаточно уметь вычислять $\nabla f$ и $\text{prox}_r$.](../files/prox_logistic.pdf)

## Восстановление матриц: ядерная норма

![Проксимальный метод с SVD мягким порогом восстанавливает матрицу ранга $r=5$ из 50\% наблюдений. Ранг итерата быстро стабилизируется около истинного. Проксимальный оператор ядерной нормы — мягкое пороговое отсечение сингулярных значений.](../files/prox_matrix_completion.pdf)
